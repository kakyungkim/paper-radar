...
## 핵심 흐름
(2026-W29 기준) Evo 2 Probes(guntoro, arXiv:2607.14070)는 게놈 언어 모델 Evo 2 기반 프로브로 메타게노믹스 데이터에서 항생제 내성(AMR, antimicrobial resistance) 유전자와 독성 기능을 스크리닝한다. 직접 임상 코호트 검증은 없으나, 병원 내 감염 모니터링과 환경 메타게노믹스 AMR 감시에 적용 가능한 경로가 가시화된다. W28 PREDIKTOR의 제로샷 약물 반응 예측에 이어 임상 근접 AI가 계속되고, W29에서는 감염병 메타게노믹스 스크리닝이 새 관심 축으로 등장했다. 전체 흐름은 in silico 평가에서 실제 임상 근접 응용으로 이동하고 있으며, 감염병·종양학·공정성 세 방향이 병행된다.
