...
- **This Week's Top 5** — 핵심 논문 5편. 각 논문에:
  ...
  - **저자**: 1저자, 교신저자 | 소속 기관 — raw 파일에서 가져옴. 누락 금지.
  - **선택 이유** 1문장 — raw 파일의 `선별 사유`를 다듬어 쓴다. "이번 주 신약 ADME 예측 방법론 중 코드 공개+재현 가능성이 가장 높아 선정" 수준. "흥미롭다" 반려.
  - 코드 공개 여부 — 코드/데이터 링크 있으면 `[code](url)`, 없으면 메타 줄에 `코드 미공개 — 재현 불가` 명시.
